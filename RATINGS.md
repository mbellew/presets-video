...
| ghost — Geiss, Skin Dots 10b (video motion reaction) | ★★★★ | Shimmering moiré ripple field with the figure dissolving through it. Reworked: it used to move on a **clock**, not the music (its only audio term was a smoothed `bass_att` on zoom; everything else was 0.03–0.05 Hz sinusoids of `time`) — hence "slow". Now a per-beat envelope drives the zoom punch, a rocking rotation, a decay flush (image clears and re-forms with the music) and the noise sparkle that forms the globs, with a raw-`bass` fallback when the beat tracker is unsure. Plus **ghost lag**: the video feed is shifted *against* the subject's horizontal speed (`seg_vx`), so the apparition flies off a moving body and dumps colour across the frame before settling back. |
| ghost color zoom — Geiss, Skin Dots 10b | — | Kept only as an A/B control. Differs from `ghost` by **one token** (`outer_halo` vs `inner_halo` — magenta rims outside vs inside the silhouette), and the name describes nothing it does. Delete once the reworked `ghost` is signed off. |
