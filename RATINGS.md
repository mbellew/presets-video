...
| recursive quad — time-offset | ★★★★ | 2026-07-11 | "Bad backup dancers": a 2x2 recursive tiling where each quadrant is seeded from a different point in the video history, so your copies echo you in canon. Reworked: alternate columns are **mirrored**, so the dancers face each other and read as a formation rather than clones; three of them keep near-time (an eighth-beat apart, jittered per run so they're human-sloppy rather than metronomic); and one — chosen at random each run — is 1.25–1.75 beats behind, landing on the off-beat so he looks like he's *trying* and missing. Room left for a colour/beauty pass. |
...
| lava-lamp (was dissolve.milk) | ★★ | 2026-07-24 | Molten lava-lamp blobs shed from moving *hands* (pose wrist capsules, gesture-shaped: thrust→streak, sweep→field), consolidated by an **MBO surface-tension** step (blur + threshold @0.5 = mean-curvature flow) then committed to RGB and drifting off; figure solid on top. The sophisticated two-layer (alpha-activation + RGB-commit) sibling of `firedancer`. **More conceptual than entertaining — not ready for the show:** the mechanism is elegant (and the "what doesn't work" list in its comments is hard-won) but the result doesn't yet land as a performance piece. No music reactivity. Renamed from `dissolve.milk`. |
| firedancer (dissolve) | ★★★★ | 2026-07-24 | Silhouette sheds molten lava-lamp blobs where a limb moves fast, re-forming when still (after Lozano-Hemmer's *Treachery of Sanctuary*). A feedback **density field**, no particles: emission at the motion-weighted seg edge, then advection down a time-warped 3D-noise gradient so density drains into the noise "wells" and clumps into rounded blobs; decay dissolves the wake; comp thresholds it into lava and draws the photographic figure solid on top. The reactive hook that made it land: **bass drives a centered scale/zoom pulse of the noise field** (`ns` scaled by `1 + bass − bass_att` about the body centroid) — punchy because it changes cell size *and* flow strength at once. Treble transients add a swirl burst (`curl*(q8+q16)`); gated bass-A (outward push, enveloped throb) / bass-B (fuzz) alternatives left in for A/B compare. Slow incommensurate x/y noise drift breaks the ~7 s z-axis loop. Still exploratory: treble swirl and the A/B bass gates are unsettled; buoyancy on. |
| machine-code-body | ★★★ | 2026-07-24 | Matrix-style falling machine code: a Gray-Scott reaction-diffusion "code" substrate revealed **only inside your silhouette** via the main-buffer **alpha channel** (advected with the flow, clamped/killed so it can't wrap), with a pose-driven pink **heart** that oversaturates alpha and plumes upward. **Close to ready** — the RD + alpha-reveal architecture is solid and the figure reads; still settling the heart plume and the (currently `0*`-disabled) bass/treble/beat reactivity. |
| comicbook | ★★★★ | 2026-07-24 | Opening titles of a comic-book movie built live from the camera — a composed comic **page** (pose-framed panels, halftone/ink look, booth/splash modes), not a flipbook. **Known glitch: the caption-texture handling** — needs a fix before it ships. |
| burning-man | ★★★★ | 2026-07-24 | "You are the Man — hit the pose and catch fire." Two-act: a cold white-on-black drawing of you, then ignition. A good **cut-scene** preset. |
